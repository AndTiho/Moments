...
class CreateMomentView(LoginRequiredMixin, CreateView):
    """
    Контролер для создания нового момента.

    Доступно только авторизованным пользователям.
    После создания запись сохраняется за текущим пользователем.
    """
...
class HomeMomentView(ListView):
    ...
    Отображает:
    - для авторизованных пользователей — их собственные моменты
    - для гостей — только публичные моменты
    """
...
class ListSelfMomentView(LoginRequiredMixin, ListView):
    ...
    def get_queryset(self):
        queryset = Moment.objects.filter(owner=self.request.user).order_by(
            "-created_at"
        )
    ...
    def get_context_data(self, **kwargs):
        ...
        context["periods"] = Moment.objects.filter(owner=self.request.user).dates(
...
class ListPublicMomentView(ListView):
    """Контролер для вывода списка публичных моментов"""
...
class SearchMomentView(ListView):
    """
    Поиск моментов по заголовку и описанию.

    Выполняет поиск:
    - среди собственных записей пользователя
    - среди публичных записей других пользователей
    """
    ...
    def get_context_data(self, **kwargs):
        ...
        if query:
            ...
            if self.request.user.is_authenticated:
                my_results = Moment.objects.filter(owner=self.request.user).filter(
                    search_q
                )
                ...
                public_results = Moment.objects.filter(is_public=True).filter(search_q)
...
class DetailMomentView(LoginRequiredMixin, DetailView):
    """
    Контролер для детального просмотра моментов.

    Отображает:
    - для своих моментов — редактировать, удалить
    - для публичных - только просмотр
    """
    form_class = MomentForm
...
class UpdateMomentView(LoginRequiredMixin, UpdateView):
    """Контролер для редактирования момента"""
    form_class = MomentForm
    template_name = "moment/moment_form.html"
...
class DeleteMomentView(LoginRequiredMixin, DeleteView):
    """Контролер для удаления момента"""
    model = Moment
